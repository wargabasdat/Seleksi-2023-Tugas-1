...
while (page <= 180) :
    ...
    for objek in objek_wisata:
        ...
        try :
            nama_jenis = objek.find('div', 'biGQs _P pZUbB hmDzD').text.replace(' • ', ',').split(',')[0]
        except :
            nama_jenis = ''
        try :
...
with open('Data Scraping/data/jenis_objek.json', 'w') as file:
...
# writer = csv.writer(open('Data Scraping/data/jenis_objek.csv', 'w', newline = ''))
