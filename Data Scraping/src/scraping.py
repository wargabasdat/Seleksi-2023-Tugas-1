...
def fetch_data(file_player,file_statistics,amount,gender,match_type):
    """Common algorithm for fetching data"""
...
def fetch_double_data(file_player,file_statistics,amount,first_gender,second_gender,match_type):
    ...
    # Load single players
    raw = open(file_player)
    players = json.load(raw)
    try:
        raw = open(file_statistics)
        double_player_statistics = json.load(raw)
    except FileNotFoundError :
        double_player_statistics =[]
        
    # Load single player statistics
    # location = os.path.join(file_name,"single_statistics.json")
    # raw = open(location)
    # player_statistics = json.load(raw)

    # Determine id
    player_id = len(players)
    player_strings = [f"{player['firstName']} {player['lastName']} {player['country']}" for player in players]
    for _ in range (amount):
        body = driver.find_element(by = By.TAG_NAME, value="tbody")
        rows = body.find_elements(by = By.TAG_NAME,value="tr")
        next_button = driver.find_element(By.XPATH,"//nav[@class='pagination']")
            ...
            try :
                ...
                if len(player_first_names) > 1:
                    ...
                    second_player_second_name = data.find_elements(By.CLASS_NAME,
                                                                "name-2")[1].get_attribute("innerHTML")
                    rank = int (data.find_element(by=By.CLASS_NAME, 
                                        value="rank-value").get_attribute("innerHTML"))
                    flag = data.find_element(by=By.TAG_NAME,value="img")
                    country = flag.get_attribute("title")
                    ...
                    if first_player not in player_strings:
                        new_player = {
                            "id" : player_id,
                            "firstName" : first_player_first_name.encode('cp1252', 'ignore').decode('cp1252'),
                            "lastName" : first_player_last_name.encode('cp1252', 'ignore').decode('cp1252'),
                            "country" : country,
                            "gender" : first_gender
                        }
                        first_id = player_id
                    ...
                    if second_player not in player_strings:
                        new_player = {
                            "id" : player_id,
                            "firstName" : second_player_first_name.encode('cp1252', 'ignore').decode('cp1252'),
                            "lastName" : second_player_second_name.encode('cp1252', 'ignore').decode('cp1252'),
                            "country" : country,
...
def scrape_single_men_players():
    ...
    men_players, men_players_statistics = fetch_data(player_file,statistic_file,17,"male","men_single")
    with open(player_file,"w") as write_file :
...
def scrape_single_women_players():
    ...
    player_file = os.path.join(data_folder,"players.json")
    statistic_file = os.path.join(data_folder,"single_statistics.json")
    women_single_players,women_single_statistics = fetch_data(player_file,statistic_file,12,"female","women_single")
    with open(player_file,"w") as write_file: 
...
def scrape_men_double():
    ...
    time.sleep(3)
    men_double_players, men_double_statistics = fetch_double_data(player_file,statistic_file,13,"male","male","men_double")
    with open(player_file,"w") as write_file : 
...
def scrape_women_double():
    ...
    statistic_file = os.path.join(data_folder,"double_statistics.json")
    time.sleep(3)
    women_double_players,women_double_statistics = fetch_double_data(player_file,statistic_file,10,"female","female","women_double")
    with open(player_file,"w") as write_file : 
...
def scrape_mixed_double():
    ...
    statistic_file = os.path.join(data_folder,"double_statistics.json")
    time.sleep(3)
    mixed_double_players,mixed_double_statistics = fetch_double_data(player_file,statistic_file,14,"male","female","mixed_double")
    with open(player_file,"w") as write_file : 
